        ...
        try {
            ...
            if ($code || (isset($options['user']) && $options['user'] === 'init')) {
                ...
                if (!$aToken && empty($code)) {
                    ...
                    // random, unguessable state value that is stored server-side
                    // (session) and must come back unchanged on the callback
                    // before any authorization code is redeemed.
                    $state = bin2hex(random_bytes(16));
                    $client->setState($state);
                    $options['csrfState'] = $state;
                    ...
                    if ($code) {
                        // The state value Google echoed back on this request:
                        // via $_GET on the direct provider redirect, or via
                        // $options['state'] once the browser relays it back to
                        // us in the second ('redirect') round-trip below.
                        $returnedState = $itpCare
                        ...
                        if (empty($expectedState) || empty($returnedState) || !hash_equals((string)$expectedState, (string)$returnedState)) {
